...
def main (texto):
    ...
    tiempo_inicial,tiempo_final=modelo(sitios,prepilas,pilas,Krecogida,m,alpha1,alpha2,alpha3,alpha4,alpha5,alpha6,alpha7,cap,captri,CFk,CFex,CFm,Cants,Cp,Cr,Tsrk,Tspk,Trpk,Tpq,Npq,verificador)

    
    tiempo_total=tiempo_final-tiempo_inicial
    print("El tiempo es de: ",round(tiempo_total,3), "segundos.")

def modelo(sitios,prepilas,pilas,Krecogida,m,alpha1,alpha2,alpha3,alpha4,alpha5,alpha6,alpha7,cap,captri,CFk,CFex,CFm,Cants,Cp,Cr,T_sr,T_sp,T_rp,Tpq,Npq,verificador):
    ...
    if verificador==1:
        ...
        for r in Nprepilas:
            modelo += (lpSum(Xsr[(s,r,k)] for s in Nsitios for k in Nkrecogida) <= 10000*lpSum(Xrp[(r,p,k)] for p in Npilas for k in Nkrecogida ) )

        for r in Nprepilas:
            modelo += ( lpSum(Xrp[(r,p,k)] for p in Npilas for k in Nkrecogida) <= 10000 * lpSum(Xsr[(s,r,k)] for s in Nsitios for k in Nkrecogida))
